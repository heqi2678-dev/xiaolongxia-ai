...
3. **第三期 视频与口型**（进行中）：视频适配器（Seedance）、口型适配器、仿真人引擎、共用合规授权区（`ui.js` 的 `complianceCard`/`bindCompliance`，两个工作台同一套）、跨工程角色库（`character.js` 的 `libAll/libSave/libFromProject/libToProject` + `ui.js` 的角色库面板）、服务端合成与素材包导出的合规闸门。
...
1. 单元：适配器请求构造与响应解析、角色提示词组装、状态机迁移、合规校验。
   - 前端测试位于 `tests/drama.test.js`，用最小 DOM/fetch 桩加载 `src/drama/*.js`，运行 `node --test tests/drama.test.js`（当前 32 项）。
2. 接口：工程 CRUD、房间隔离（两个账号交叉验证）、合成接口、发布接口。
3. 端到端：手搓台从剧本到导出；半自动台从题材到发布；失败镜重试路径。
   - 真 DOM 端到端位于 `tests/drama-e2e.js`，用 jsdom 提供真实 DOM/事件/localStorage，加载 `src/drama/*.js` 后按用户操作点按钮、填表单，网络全部打桩（166 项断言，5 条链路）。
   - 运行：`NODE_PATH=/usr/local/lib/node_modules node tests/drama-e2e.js`（需全局安装 `jsdom`）。
4. 合规：导出产物含显式与隐式标注；无授权时禁止真人形象导出。
5. 回归：店门 33 个测试与既有页面烟测。
