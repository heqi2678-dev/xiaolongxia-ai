...
1. **第一期 底座**：`src/drama/` 目录、适配器框架、工程模型、本地与服务器存储接口、设置页短剧服务分组。
2. **第二期 图像与配音**：图像适配器、语音适配器、角色卡与一致性、手搓台主体。
3. **第三期 视频与口型**：视频适配器（Seedance）、口型适配器、仿真人引擎。
...
1. 单元：适配器请求构造与响应解析、角色提示词组装、状态机迁移、合规校验。
   - 前端测试位于 `tests/drama.test.js`，用最小 DOM/fetch 桩加载 `src/drama/*.js`，运行 `node --test tests/drama.test.js`（当前 18 项）。
