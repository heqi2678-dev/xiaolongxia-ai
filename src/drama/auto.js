/* 铜龙电商 · AI 短剧工作台 · 半自动台「分镜流水线」 */
(function () {
  const D = XLX.drama;
  const U = XLX.util;
  const state = { stage: "input", project: null, plan: null, busy: false, lastComposed: null, input: { topic: "", genre: "comic", shotCount: 6, style: "cn-manhua" } };
  ...
  async function render() {
    ...
    const v = view();
    if (!v) return;
    v.innerHTML = '<div class="dw-wrap">' + stepper() + '<div id="dwAutoBody"></div><div id="dwAutoMsg"></div></div>';
  ...
  function stepper() {
    ...
    return '<div class="dw-steps">' + STAGES.map((s, i) =>
      '<span class="dw-step' + (i === idx ? " on" : (i < idx ? " done" : "")) + '">' + (i < idx ? "✓ " : "") + s[1] + "</span>"
    ).join("") + "</div>";
  }
  ...
  function renderInput(body) {
    ...
    body.innerHTML = '<div class="dw-card"><h3>输入题材，AI 帮你出剧本分镜</h3>' +
      ...
      "</div>" +
      '<div class="dw-bar" style="margin-top:12px"><button class="btn primary" id="auGo">AI 出剧本分镜</button>' +
      '<button class="btn small" id="auGuide">看教程</button></div>' +
  ...
  async function runPlan() {
    ...
    try {
      ...
      state.plan = plan;
      const p = D.project.blank({ title: (state.input.topic || "AI 短剧").slice(0, 18), genre: state.input.genre, source: "auto" });
  ...
  function renderPlan(body) {
    ...
    "</div>" +
    '<div class="dw-card"><h3>分镜草案（可改提示词与台词）</h3><div id="auShots"></div></div>' +
    ...
    const box = document.getElementById("auShots");
    const handlers = {
      move: (sid, b) => { D.project.moveShot(p, sid, Number(b.dataset.dir)); D.project.save(p); refresh(); },
      remove: (sid) => { D.project.removeShot(p, sid); D.project.save(p); refresh(); },
      onChange: () => D.project.save(p)
    };
    function refresh() { D.ui.renderShots(box, p, handlers); D.ui.bindShots(box, p, handlers); }
    refresh();
    ...
    v.querySelector("#auAdd").onclick = () => { D.project.addShot(p); D.project.save(p); refresh(); };
  ...
  function renderGen(body) {
    ...
    body.innerHTML = '<div class="dw-card"><h3>批量生成</h3>' +
      '<div class="dw-hint">正在逐镜生成，请保持页面打开。失败的单镜可在下一关重绘。</div>' +
      '<div class="dw-bar" style="margin-top:10px"><button class="btn primary" id="auGenRun">开始生成</button>' +
      '<button class="btn ghost danger" id="auGenStop">停止</button>' +
      '<button class="btn" id="auGenNext">跳过，去检查</button></div>' +
      '<div id="auGenList"></div>' +
    "</div>";
    document.getElementById("auGenList").innerHTML = (p.shots || []).map(s =>
      '<div class="dw-shot ' + s.status + '" data-card="' + s.id + '"><div class="dw-shot-head"><span class="dw-seq">第 ' + s.seq + ' 镜</span><span data-status="' + s.id + '">' + D.ui.statusBadge(s) + '</span><div class="dw-progress" data-prog="' + s.id + '"></div></div></div>'
    ).join("");
    const v = view();
  ...
  function renderReview(body) {
    ...
    body.innerHTML = '<div class="dw-card"><h3>关卡二 · 逐镜检查</h3>' +
      '<div class="dw-hint">不满意的单镜点「重绘」。全部满意后通过。</div></div>' +
      '<div class="dw-card"><div id="auReviewShots"></div>' +
      '<div class="dw-bar"><button class="btn" id="auRedrawFail">重绘全部失败镜</button>' +
      '<button class="btn primary" id="auApprove2">通过，去配音</button></div></div>';
    const box = document.getElementById("auReviewShots");
    const handlers = {
      gen: (sid) => redraw(sid),
  ...
  async function redrawTts(sid) {
    ...
    try { D.ui.progress(sid, "配音中…"); await D.engine.synthShot(p, shot); if (D.engine.isRealistic(p) && shot.videoUrl) await D.engine.lipsyncShot(p, shot); await D.project.save(p); D.ui.refreshShot(p, shot); D.ui.progress(sid, ""); }
  ...
  async function redrawLipsync(sid) {
    ...
    try { D.ui.progress(sid, "口型中…"); await D.engine.lipsyncShot(p, shot); await D.project.save(p); D.ui.refreshShot(p, shot); D.ui.progress(sid, ""); }
  ...
  function pickShotCover(sid) {
    ...
    input.onchange = async () => {
      ...
      await D.project.save(state.project); D.ui.refreshShot(state.project, shot);
    };
    input.click();
  }
  ...
  function renderVoice(body) {
    ...
    body.innerHTML = '<div class="dw-card"><h3>配音与字幕</h3>' +
      '<div class="dw-hint">需要补配音的分镜：' + need + ' 个。字幕会在合成时按台词自动生成。</div>' +
      '<div class="dw-bar" style="margin-top:10px"><button class="btn primary" id="auTtsRun">批量配音</button>' +
      '<button class="btn" id="auApprove3">通过，去合成</button></div>' +
      '<div id="auVoiceList"></div></div>';
    const v = view();
  ...
  function renderFinal(body) {
    ...
    v.querySelector("#auRestart").onclick = () => {
      state.project = null; state.lastComposed = null; state.stage = "input";
      render();
    };
  }
